...
Unlike existing lattice-based schemes that treat compression and rounding as secondary implementation details, AFEOL-RELC elevates rounding to a first-class cryptographic primitive with explicit error bounds, exact empirical statistics, and formally justified correctness and security guarantees.

All correctness, failure probability, and CCA-security guarantees in AFEOL-RELC are derived **exclusively from verifiable rounding error statistics obtained by exhaustive enumeration**, and **not** from idealized quantization models or continuous noise assumptions.
...
- mathematically closed with respect to all noise sources,
- fully transparent in correctness and failure analysis,
- compatible with the Fujisaki–Okamoto (FO) transform in the QROM.
...
|------|----------------------|-------------------------------|-------------------------------|-------------|
| 512 | 3 | 3.617334848… | 1.616… | \( \approx 9 \times 10^{-4} \) |
| 1024 | 2 | 0.924315842… | 0.770… | \( \approx 6 \times 10^{-4} \) |
| 8 | 208 | 14430.04218… | 104.029… | \( \approx 6 \times 10^{-2} \) |

**Proof:** Exhaustive enumeration.

### 4.2 Normative Use of Statistics

All statistical quantities used by AFEOL-RELC are **population statistics** derived from Lemma 1.

Analytic, continuous, or idealized quantization formulas are **non-normative** and MUST NOT replace these values.

Since \( \mathrm{Var}(\varepsilon_M) \gg \mu_M^2 \) for all \( M \), all concentration bounds safely normalize \( \mathbb{E}[\varepsilon_M] = 0 \).
...
\[
w = \langle \mathrm{Lift}_{p_{pk} \to q}(b), r \rangle + e_2
\]
...
Message embedding (binary message):
\[
...
**Informative note:**  
For RELC-768R parameters, the expected hint size is approximately **32–64 bytes per ciphertext**, based on empirical distributions.
...
\[
w' = \langle \mathrm{Lift}_{p_{ct} \to q}(u), s \rangle
\]
...
\[
\hat{w}'_{\mathrm{corr},i} =
\begin{cases}
\hat{w}'_i + \mathrm{sign}(\mathrm{center}_T(\hat{w}'_i)) \cdot T,
& i \in \text{hint} \ \wedge \ |\mathrm{center}_T(\hat{w}'_i)| \ge \tau \\
...
m_i =
\left\lfloor
\frac{2}{T} \cdot \mathrm{center}_T(v_i - \hat{w}'_{\mathrm{corr},i})
\right\rceil \bmod 2
\]
...
Rounding errors \( \varepsilon_b \) and \( \varepsilon_u \) are deterministic.
...
The hint is a deterministic function of \( \hat{w} \).
...
- Dimension: 768  
- Classical security: \( 2^{134} \)–\( 2^{138} \)  
- Quantum security: \( 2^{67} \)–\( 2^{69} \)  
- NIST level: ≥ 1
...
- Lower correctness margin than ML-KEM-768
...
AFEOL-RELC defines a new class of **rounding-explicit lattice cryptographic constructions** in which rounding is fully formalized, measured, and auditable.

All correctness and security guarantees are derived directly from verified rounding behavior, providing a transparent and reproducible foundation for future lattice-based cryptographic designs.
...
Do **NOT** deploy AFEOL-RELC in production systems without independent cryptographic review and certification.  
For production use, rely on NIST-standardized schemes such as ML-KEM / Kyber.
